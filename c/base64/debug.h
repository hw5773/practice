...
#define DPI_DEBUG_ALL               DPI_DEBUG_MAIN|DPI_DEBUG_SERVER|DPI_DEBUG_CLIENT|DPI_DEBUG_MIDDLEBOX
  ...
  #define fstart(format, ...) printf("[BASE64/FINFO] Start: %s:%s: " format "\n", __FILE__, __func__, ## __VA_ARGS__)
  #define ffinish(format, ...) printf("[BASE64/FINFO] Finish: %s:%s: " format "\n", __FILE__, __func__, ## __VA_ARGS__)
  #define ferr(format, ...) printf("[BASE64/FINFO] Error: %s:%s: " format "\n", __FILE__, __func__, ## __VA_ARGS__)
#else
  ...
  #define imsg(type, format, ...) if (dtype & type) printf("[BASE64/INFO] %s:%s: " format "\n", __FILE__, __func__, ## __VA_ARGS__)
  #define iprint(type, msg, buf, start, end, interval) \
    if (dtype & type) { \
      do { \
        int i; \
        printf("[BASE64/INFO] %s:%s: %s (%d bytes)\n", __FILE__, __func__, msg, end - start); \
        for (i = start; i < end; i++) \
        { \
          printf("%02X ", buf[i]); \
          if (i % interval == (interval - 1)) \
  ...
  #define emsg(format, ...) printf("[BASE64/ERROR] " format "\n", ## __VA_ARGS__)
#else
  #define emsg(format, ...)
#endif /* ERROR */
